...
│                                       │                      │
│  ┌──────────────────────────────┐    │  ┌────────────────┐  │
│  │ 硬件选择 [A100 80GB ▼]       │    │  │ 最大并发数     │  │
│  └──────────────────────────────┘    │  │ ────────────── │  │
│                                       │  │                │  │
│  ┌──────────────────────────────┐    │  │ 不考虑 PA: N   │  │
│  │ 模型选择 [Llama-2-7b ▼]      │    │  │ 考虑 PA ×2.3:  │  │
│  └──────────────────────────────┘    │  │          M     │  │
│                                       │  └────────────────┘  │
│  ───────────────                      │                      │
│                                       │  ─────────────────   │
...
| 硬件选择 | Select | 空 |
| 模型选择 | Select | 空 |
| 上下文长度 | Input (number) | 4096 |
| 框架占用 | Input (number) + Select | 2GB vLLM |
| 计算精度 | Radio Group | FP16 |

**右侧结果区域：**
- **最大并发数卡片**：显示两个数值（不考虑 Paged Attention / 考虑 Paged Attention）
- **显存占用明细表格**：包含单并发显存和总显存
...
| Paged Attention | vLLM 提出的分页注意力机制，减少 KV Cache 显存碎片化 |
